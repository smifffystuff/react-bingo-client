...
export const gerenateCards = () => {
  ...
  while (failed) {
    ...
    for (let r = 0; r < 18; r++) {
      const row = []
      const availableCols = [0,1,2,3,4,5,6,7,8].filter(c => availableNumbers[c].length > 0)
      ...
      for (let i = 0; i < 5; i++) {
        ...
        const numberIndex = Math.floor(Math.random() * availableNumbers[thisCol].length)
  ...
  for (let r = 0; r < 18; r++) {
    ...
    playerCards[cardNum].rows.push({id: uuid(), numbers: rows[r], remaining: 5, called: []})
  }
  return playerCards;
};
